...
export default withColors({ backgroundColor: 'background' })(({
	attributes,
	setAttributes,
	backgroundColor,
	setBackgroundColor
	...
	const blockProps = useBlockProps({
		style: blockStyle,
		className: `iis-block-hero ${(!displayImage && !displayYoutube && !displayVideo) ? 'iis-block-hero--no-image' : ''} ${(displayVideo || displayYoutube) ? 'iis-block-hero--video' : ''} ${(attributes.layout === 'dynamic') ? 'iis-block-hero--dynamic' : ''}`,
	});
